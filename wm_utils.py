...
# add by Ernest
from df_to_excel import df_update_xlsx, wafer_content
from excel_styler import wafer_map_styler
...
def coord_to_grid(coord, die_size, grid_center, flip_y):
    """
...
def grid_to_rect_coord(grid, die_size, grid_center, flip_y=False):
    """
    ...
    if flip_y:
        ...
        _y = die_size[1] * (grid[1] - grid_center[1] - 0.5)
    return _x, _y
...
class ContinuousColorFunction:
    def __init__(self, df_map, plot_range=None):
        import wm_constants as wm_const
        self.high_color = wm_const.wm_HIGH_COLOR
        self.low_color = wm_const.wm_LOW_COLOR
        self.oor_high_color = wm_const.wm_OOR_HIGH_COLOR,
        self.oor_low_color = wm_const.wm_OOR_LOW_COLOR
        self.invalid_color = wm_const.wm_INVALID_COLOR
        self.gradient = LinearGradient(self.low_color, self.high_color)
        self.plot_range = plot_range
        if self.plot_range is None:
            xyd = list(df_map.itertuples(index=False))  # add by Ernest
            p_98 = float(nanpercentile([_i[2]
                                        for _i
                                        in xyd], 98))
            p_02 = float(nanpercentile([_i[2]
                                        for _i
                                        in xyd], 2))

            data_min = min([die[2] for die in xyd])
            data_max = max([die[2] for die in xyd])
...
def flat_find_and_flip(df_map, export_excel=False):
    up = (df_map.Y == df_map.Y.max()).sum()
    down = (df_map.Y == df_map.Y.min()).sum()
    if export_excel:
        df = df_map.copy()
        if up > down:
            arr = df.Y.unique()
            upside_down_dict = dict(zip(arr, arr[::-1]))
            df['Y'] = df['Y'].map(lambda a: upside_down_dict[a])
        return df
    else:
        return up > down  # True need to flip wafer upside down


def export_to_excel(df_map=None, color_func=None, filename=None, die_size=None, tab_title='', first=True, plot_range=None,
                    wafer_map=True):  # add by Ernest

    df_map = flat_find_and_flip(df_map, export_excel=True)
    if wafer_map:
        if color_func is None:  # direct call by UI
            c = ContinuousColorFunction(df_map, plot_range=plot_range)
            color_func = c.get_color
        df_data_map = df_map.pivot(index='Y', columns='X')['DATA']
        df_data_map = df_data_map.reindex(index=df_data_map.index[::-1])
        df_data_map.reset_index(inplace=True, drop=True)

        filename = df_update_xlsx(df_data_map, filename, f'{tab_title}MAP', styler=wafer_map_styler,
                                  apply_axis=None, offset=0, fit_col_width=False, cell_width_height=wafer_content,
                                  cell_width_height_kwargs={'size': die_size},
                                  apply_kwargs={'color_func': color_func}, index=False,
                                  header=False, truncate_sheet=True, return_writer=True)
    df_map.rename(columns={'DATA': tab_title}, inplace=True)
    if not first:
        df_map.drop(columns=['X', 'Y'], inplace=True)
    return filename, df_map


def data_export_to_excel(df_list, filename, return_writer=True):
    df = pd.concat(df_list, axis=1)
    if return_writer:
        filename = df_update_xlsx(df, filename, 'DATA', fit_col_width=False, index=True, offset=0, header=True,
                                  truncate_sheet=True, return_writer=True, new_sheet_position=0)
        return filename
    else:
        return df
