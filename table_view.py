...
class ImageView(TableView):

    def __init__(self, image_columns: list, image_size_h=160, image_size_w=90, df=pd.DataFrame(), title="QWQ",
                 parent=None, init_show=False):
        """

        :param image_columns: column index that store image path
        :param df: imput df
        :param title: title
        :param parent: parent
        :param init_show: run it by itself true to show UI
        """
        super(ImageView, self).__init__(init_show=init_show, df=df, title=title, parent=parent)
        self.image_columns = image_columns
        self.image_size_h = image_size_h
        self.image_size_w = image_size_w
        for col in self.image_columns:
            self.setItemDelegateForColumn(col, ImageDelegate(parent))

    def sizeHint(self, option, index):
        return QtCore.QSize(self.image_size_h, self.image_size_w)
...
class BinSelectionView(TableView):
    def __init__(self, query_func, worker, df=pd.DataFrame(), title="Select your want", parent=None, init_show=False,
                 merge_cell=True, cp_stages=None, insp_deps=None):
        super(BinSelectionView, self).__init__(init_show=init_show, df=df, title=title, parent=parent)
        self.worker = worker
        if cp_stages is None:
            cp_stages = ['CP3']

        if insp_deps is None:
            insp_deps = ['MFG']

        self.cp_stages = cp_stages
        self.insp_deps = insp_deps
        self.search_hint_cp = QtWidgets.QLabel()
        self.search_hint_cp.setText('Select test type:')
        self.search_hint_cp.setAlignment(QtCore.Qt.AlignBottom | QtCore.Qt.AlignLeft)

        self.search_hint_insp = QtWidgets.QLabel()
        self.search_hint_insp.setText('Select inspection department:')
        self.search_hint_insp.setAlignment(QtCore.Qt.AlignBottom | QtCore.Qt.AlignLeft)

        self.ui.tableView.setSelectionBehavior(QAbstractItemView.SelectRows)

        self.combo_box_cp = QtWidgets.QComboBox(self)
        self.combo_box_cp.addItems(cp_stages)
        self.combo_box_cp.setCurrentIndex(2)

        self.combo_box_insp = QtWidgets.QComboBox(self)
        self.combo_box_insp.addItems(insp_deps)
        self.combo_box_insp.setCurrentIndex(0)

        self.ui.verticalLayout_2.insertWidget(1, self.search_hint_cp)
        self.ui.verticalLayout_2.insertWidget(2, self.combo_box_cp)
        self.ui.verticalLayout_2.insertWidget(3, self.search_hint_insp)
        self.ui.verticalLayout_2.insertWidget(4, self.combo_box_insp)

        self.search_hint2 = QtWidgets.QLabel()
        self.search_hint2.setText('Select bin code for part to inspect, default is all')
        self.search_hint2.setAlignment(QtCore.Qt.AlignBottom | QtCore.Qt.AlignLeft)
        self.ui.tableView.setSelectionBehavior(QAbstractItemView.SelectRows)
        self.ui.verticalLayout_2.insertWidget(5, self.search_hint2)
    ...
    def ok(self):
        ...
        if len(selected_idx) > 0:
            ...
            set_of_not_query = side_set.difference(set(df['Side'].unique()))
            selected_bin_codes = {side_d[side]: tuple() for side in set_of_not_query}
        kwargs = selected_bin_codes
        kwargs['cp_stage'] = str(self.combo_box_cp.currentText())
        kwargs['insp_dep'] = str(self.combo_box_insp.currentText())
        self.worker.fetch(self.query_func, kwargs=kwargs)
