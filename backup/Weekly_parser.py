...
from Database_query import weekly_lot, fsbs_insp, cp, scrap_lot
...
def query_product_group(part):
    for i in Tg.active_parts_and_product_group.items():
        ...
        raise ValueError("part doesn't belong to any group like SMP, Conti, GEN1, ....etc")
    return group
...
def query_wf_data(part, wf, ignore_db_error, child_part, parent_part):
    # get insp data
    insp_data = query_data_by_func(part, wf, ignore_db_error, parent_part, child_part, fsbs_insp)
    # get cp data
    cp_data = query_data_by_func(part, wf, ignore_db_error, parent_part, child_part, cp)
...
def get_previous_data(part="", sheet_name=None, row_limit=None, search_week=True, ignore_index=True, bsl=True,
...
if __name__ == '__main__':
    ...
    #     # get insp data
    #     if child_part:
    #         insp_data = Tg.summary_cache.get(('QueryingCache', part[:5], wf, 'insp'), fsbs_insp, wf)
    #     else:
    #         if ignore_db_error:
    #             try:
    #                 insp_data = fsbs_insp(wf)
    #             except Exception as e:
    #                 Tg.update_all_report_until_error_log.append(str(e))
