...
inline_prompts_lst = ['FSINSP_SYL']  # lots that pass these DCOP item Name will count in weekly report

connstr = 'DRIVER={};SERVER={};UID={};PWD={}'.format(driver, server, user, pwd)

gross_die_modified_dict = cfg['gross die by part']
id_flag = re.compile("^1[A-Z][A-Z0-9][A-Z]\d{3}\.\d{1,2}$")
part_flag = re.compile("^[A-Z]{2}[A-Z0-9]{3}\d{0,}$")
lot_flag = re.compile("^1[A-Z][A-Z0-9][A-Z]\d{3}")
...
class MapDataNotFoundError(Exception):
    def __init__(self, _part_id):
        self.value = "Can't find the map data of {}.".format(_part_id)
...
class CPDataNotFoundError(Exception):
    def __init__(self, _id):
        self.value = "Can't find the cp data of {} (wafer_id). Please check the lot history or input parameter.".format(_id)
...
class GrossDieNotFoundError(Exception):
    def __init__(self, _part):
        self.value = "Can't find the cp gross die of {}. Please check the part.".format(_part)
...
def cp(wafer_id, cp_stage='CP3'):
    """Query CP3 data by wafer_id"""
    # con = pyodbc.connect(connstr)
    if verify_id(wafer_id):
        lot_id = wafer_id.split('.')[0] + '%'
        wafer_n = wafer_id.split('.')[1]
        sql = "SELECT BIN, count(BIN) FROM (" \
              "Select A.BIN From EDADBA.CP_DATAD A, EDADBA.CP_DATAW B " \
              "Where A.Link=B.Link " \
              "AND B.LOTID LIKE '{}' " \
              "AND B.WAFER = {} " \
              "AND B.TESTTYPE = '{}') GROUP BY BIN".format(lot_id, wafer_n, cp_stage)
        df = pd.read_sql(sql, con, index_col='BIN')
        if df.empty:
            if cp_stage == 'CP3':
                df = cp(wafer_id, cp_stage='CP2')
            elif cp_stage == 'CP2':
                df = cp(wafer_id, cp_stage='CP1')
            else:
    ...
    if verify_part(part):
        ...
        if df.empty:
            if cp_stage == "CP3":
                ...
                raise MapDataNotFoundError(part)
...
def insp_map(wafer_id, side='1', bin_codes_tup='all', ensure_not_empty=False):
    """Query inspection map by wafer_id
        side '1' >> FS
        side '2' >> BS
    """
    ...
    if bin_codes_tup:
        if isinstance(bin_codes_tup, tuple):
            ...
            key = ''
        if verify_id(wafer_id):
            sql = f"""select x, y, cat from EDADBA.MAP_DATAD
            where link in(select link from EDADBA.MAP_DATAW where wafer = '{wafer_id}' and faildie='{side}') {key}"""
            df = pd.read_sql(sql, con)
...
def weekly_lot(end_date, start_date=None, drop_ep_lot=True, check_point=None):
    """
    ...
    if check_point is None:
        prompts_tup = tuple(inline_prompts_lst)
    else:
        prompts_tup = tuple(check_point)
    if len(prompts_tup) == 1:
        prompts_tup = "('{}')".format(prompts_tup[0])
...
def wafer_history(_id, search_wafer_history=True):
    if verify_id(_id):
        ...
        sql = f"""SELECT C.CURPRCDCURINSTNUM, C.STAGE, C.EQPID, C.CURMAINQTY,
                  ...
                  {sql_str} 
                  AND A.COMPSTATE IN (1,7,8,9,11,13) 
                  AND A.LOTID=B.LOTID 
    ...
    df.rename(columns={'STAGE': 'Stage', 'CURMAINQTY': 'LotQty', 'EQPID': 'Equipment',
              ...
              # index={'CURPRCDCURINSTNUM': 'InstNo', 'CURPRCDID': 'Procedure'},
              inplace=True)
...
def equipment_lot_history(eqp_id, end_date, start_date=None):
    ...
    sql = f"""SELECT A.*,C.COMPIDS FROM
              ...
              WHERE TRACKINTIME>=DATE '{str(start_date)}' 
              AND TRACKINTIME<DATE'{str(end_date)}'
              AND EQPID = "{eqp_id}" AND TRACKOUTTIME IS NOT NULL
...
def wafer_to_lot(waferid):
    ...
    if verify_id(waferid):
        sql = "SELECT LOTID, COMPIDS FROM PLLDBA.ACTL_COMPONENTS " \
              "WHERE COMPIDS = '{}' " \
              "AND COMPSTATE = 1".format(waferid)
        crsr = con.cursor()
        crsr.execute(sql)
        result = crsr.fetchone()[0]
        # con.close()
