...
def insp_pareto(df_input, side, rot90=False, return_df_sorted_data=False):
    """
    ...
    plt.title('{} Yield Loss Pareto'.format(side), fontsize=12)
...
def stacked_bar_chart(df_input, side, title=None):
    df_plot = df_input.set_index(('DATETIME', 'week'))[side]
    if title is None:
        title = '{} Yield Loss Trend'.format(side)
...
def weekly_yield(df_input):
    y_ign_bse = False
    title = 'Weekly Yield Trend'
...
def weekly_yield_loss(df_input, color=None):
    if color is None:
        color = bsvcolors
    ...
    title = 'Weekly Final Yield Loss Breakdown'
...
class ChartIOGen:
    ...
    def weekly_defect_data(self, kind, site=None, show=False, return_sorted_data=False):
        ...
        yields = {'yield': weekly_yield, 'yield_loss': weekly_yield_loss}
        if site is None:
            gen_func = yields[kind]
            plot = gen_func(self.df)
        else:
            gen_func = insp[kind]
            if return_sorted_data and kind == 'pareto':
                plot, sorted_data = gen_func(self.df, site, return_df_sorted_data=return_sorted_data)
                return img_setup(plot, image_data, show), sorted_data
            plot = gen_func(self.df, site)
