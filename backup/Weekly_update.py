...
from Database_query import connection, scrap_query_parts
...
plots = {'Weekly Yield Trend': ('yield', None), 'Weekly Final Yield Loss Breakdown': ('yield_loss', None),
...
def update_all_report_until(target_date=None, end_date=None, comparing_to_the_last_year=False, start_date=None,
    ...
    while True:
        ...
        if not parts_list:
            ...
            concurrent_thread_run(weekly_updater, parts_list, tuple((target_date, week_info, cross_year, cross_month)),
                                  3)
...
def main():
    ...
    update_all_report_until(target_date=dt.date(2018, 7, 31), end_date=dt.date(2020, 10, 16))
    # update_all_report_until(target_date=dt.date(2017, 2, 3))
...
def main1():
    writer = weekly_summary_writer('AP196', date=today)
    writer.save()
