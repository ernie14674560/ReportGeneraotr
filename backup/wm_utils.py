...
def export_to_excel(df_map, color_func, filename, die_size, return_writer=False, wafer='', plot_range=None):  # add by Ernest
    ...
    filename = df_update_xlsx(df_data_map, filename, f'{wafer}MAP', styler=wafer_map_styler,
    ...
    filename = df_update_xlsx(df_map, filename, f'{wafer}DATA', fit_col_width=False,
                              index=True, offset=0, header=True, truncate_sheet=True, return_writer=True)
    if return_writer:
        return filename
    else:
        filename.save()
