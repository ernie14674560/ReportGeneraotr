...
class ContinuousLegend(wx.Panel):
    ...
    def __init__(self,
        ...
        #     self.xyd = list(df_map.itertuples(index=False))  # add by Ernest
    ...
    def calc_ticks(self):
        ...
        ticks = []
        unit = self.wafer_detail.get('unit')
        for tick in tick_values:
            if unit is not None:
                string = unit.format(tick)
            else:
...
class DiscreteLegend(wx.Panel):
    ...
    def _init_ui(self):
        ...
        self.hbox = wx.BoxSizer(wx.HORIZONTAL)
        self.grid = MyGrid(parent=self)

        # set row hight to fit with color select column
        row_size_info = wx.grid.GridSizesInfo(20, [])
        self.grid.SetRowSizes(row_size_info)

        self.fgs = wx.FlexGridSizer(rows=self.n_items, cols=1, vgap=0, hgap=2)
        self.grid.CreateGrid(self.n_items, 5)
        for _i, (key, value) in enumerate(zip(self.labels, self.colors)):
            self.side = self.wafer_detail['bin'][str(key)]['side']
            self.label = label = str(key)
            # des = self.wafer_detail['bin'][str(key)]['description'].split(', ')[0]
            self.description = self.wafer_detail['bin'][str(key)]['description']
            self.count = self.wafer_detail['bin'][str(key)]['count']
            self.weight = self.wafer_detail['bin'][str(key)]['weight']
            ...
            self.grid.SetCellValue(_i, 0, self.side)
            self.grid.SetCellValue(_i, 1, self.label)
            self.grid.SetCellValue(_i, 2, self.description)
            self.grid.SetCellValue(_i, 3, self.count)
            self.grid.SetCellValue(_i, 4, self.weight)
            self.fgs.Add(self.colorbox)
        # Auto fit column except description
        for col in [0, 1, 3, 4]:
        ...
        #
        #
        # self.fgs = wx.FlexGridSizer(rows=self.n_items, cols=6, vgap=0, hgap=2)
        # for _i, (key, value) in enumerate(zip(self.labels, self.colors)):
        #     self.side = wx.StaticText(self,
        #                               label=self.wafer_detail['bin'][str(key)]['side'],
        #                               style=wx.ALIGN_LEFT,
        #                               )
        ...
        #                                )
        #     # des = self.wafer_detail['bin'][str(key)]['description'].split(', ')[0]
        #     des = self.wafer_detail['bin'][str(key)]['description']
        #     self.description = wx.StaticText(self,
        #                                      label=des,
        #                                      style=wx.ALIGN_LEFT,
        #                                      )
        #     self.count = wx.StaticText(self,
        #                                label=self.wafer_detail['bin'][str(key)]['count'],
        #                                style=wx.ALIGN_LEFT,
        #                                )
        #     self.weight = wx.StaticText(self,
        #                                label=self.wafer_detail['bin'][str(key)]['weight'],
        #                                style=wx.ALIGN_LEFT,
        #                                )
        #     self.colorbox = csel.ColourSelect(self,
        #                                       _i,
        ...
        #     self.Bind(csel.EVT_COLOURSELECT, self.on_color_pick, id=_i)
        #
        #     self.fgs.Add(self.side, flag=wx.ALIGN_CENTER_VERTICAL)
        #     self.fgs.Add(self.label, flag=wx.ALIGN_CENTER_VERTICAL)
        #     self.fgs.Add(self.description, flag=wx.ALIGN_CENTER_VERTICAL)
        #     self.fgs.Add(self.count, flag=wx.ALIGN_CENTER_VERTICAL)
        #     self.fgs.Add(self.weight, flag=wx.ALIGN_CENTER_VERTICAL)
        #     self.fgs.Add(self.colorbox)
        #
        #
        #
    ...
    def on_color_pick(self, event):
        ...
        #        print(event.GetId())
        self.colors[event.GetId()] = event.GetValue()
