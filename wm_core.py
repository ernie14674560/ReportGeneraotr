...
import wm_legend
import wm_utils
from aoi_viewer import AOIViewer
...
class WaferMapPanel(wx.Panel):
    ...
    def __init__(self,
        ...
        self.wafer_detail = self.wafer_info.wafer_detail
        self.flip_y = self.wafer_info.flip_y
        self.parent = parent
        self.xyd = self.wafer_info.xyd
        # backwards compatability
        if isinstance(self.wafer_info.data_type, str):
            self.wafer_info.data_type = wm_const.DataType(self.wafer_info.data_type)
        self.data_type = self.wafer_info.data_type
        self.df_map = self.wafer_info.df_map
        self.high_color = self.wafer_info.high_color
        self.low_color = self.wafer_info.low_color
        self.grid_center = self.wafer_info.center_xy
        self.die_size = self.wafer_info.die_size
        self.plot_range = self.wafer_info.plot_range
        self.plot_die_centers = self.wafer_info.plot_die_centers
        self.discrete_legend_values = self.wafer_info.discrete_legend_values
        self.discrete_legend_colors = self.wafer_info.discrete_legend_colors
        self.die_gridlines_bool = self.wafer_info.show_die_gridlines
        self.show_wafer_objects = self.wafer_info.show_wafer_objects
        self.tab_name = self.wafer_info.tab_name
        ### Other Attributes ################################################
    ...
    def _bind_events(self):
        ...
        self.canvas.Bind(FloatCanvas.EVT_RIGHT_DOWN, self.on_mouse_right_click)  # add by Ernest
    ...
    def _create_legend(self):
        ...
        if self.data_type == wm_const.DataType.DISCRETE:
            ...
            self.legend = wm_legend.DiscreteLegend(self,
                                                   ...
                                                   wafer_detail=self.wafer_detail
                                                   )
        else:
            if self.plot_range is None:
                ...
                if p_02 and p_98:  # add by Ernest, ensure not divide by 0
    ...
    def draw_die_center(self):
        ...
        for die in self.xyd:
            ...
            circ = FloatCanvas.Circle(lower_left_coord,
                                      0.5,
                                      FillColor=wm_const.wm_DIE_CENTER_DOT_COLOR.GetIM(),
                                      )
    ...
    def on_mouse_right_click(self, event):
        ...
        # Create menu
        if self.data_type == wm_const.DataType.DISCRETE:
            entries = list(self.wafer_detail['AOI folder'])
        ...
        die_coord = f'Coordinate:(Y:{dc_y} X:{dc_x})'
        die_val = self.xyd_dict.get(die_grid, False)
        if die_val:
            self.popupmenu = wx.Menu()
            if self.data_type == wm_const.DataType.DISCRETE:
                self.popupmenu.Append(-1, 'Code: {}'.format(die_val))
            else:
                self.popupmenu.Append(-1, 'Value: {}'.format(die_val))
            self.popupmenu.Append(-1, die_coord)
            self.popupmenu.AppendSeparator()
            if self.data_type == wm_const.DataType.DISCRETE:
                for entry in entries:
                    menu_id = wx.NewIdRef()
                    item = self.popupmenu.Append(menu_id, entry)

                    wrapper = lambda event: self.on_open_aoi_img(event, (dc_y, dc_x))
                    # wrapper = lambda event: print(entry, dc_y, dc_x, menu_id)
                    self.Bind(wx.EVT_MENU, wrapper)
    ...
    def on_open_aoi_img(self, event, yx):
        item = self.popupmenu.FindItemById(event.GetId())
        text = item.ItemLabelText
        if 'Bin:' in text or 'Coordinate:' in text:  # bin code
            return
        try:
            path = self.wafer_detail['AOI folder'][text]
            AOIViewer(path, yx, parent=self, df_wafer=self.wafer_detail['df_map'])
        except Exception as e:
            print(e)
            return

    def onTakeSnapShot(self, event, path):
        ...
        fileName = f"{path}/{self.wafer_info.tab_name}.png"
        img.SaveFile(fileName, wx.BITMAP_TYPE_PNG)
        print(f'saving {self.wafer_info.tab_name} as png!')
    ...
    def on_mouse_move(self, event):
        ...
        # except KeyError:
        #     die_val = "N/A"
        die_val = self.xyd_dict.get(die_grid, "N/A")
        ...
        if not die_val == "N/A":
            if self.data_type == wm_const.DataType.DISCRETE:
                aoi_hint = ':: Right click to open AOI image.'
            else:
                aoi_hint = '.'
        else:
            aoi_hint = '.'
        status_str = "Die {d_grid} :: Radius = {d_rad:0.3f} :: Value = {d_val}   Mouse {m_coord} :: Radius = {m_rad:0.3f}{aoi_hint}"
...
def draw_wafer_outline(dia=150, excl=5, flat=None):
    ...
    circ = FloatCanvas.Circle((0, 0),
                              dia,
                              LineColor=wm_const.wm_OUTLINE_COLOR.GetIM(),
                              LineWidth=1,
                              )
    ...
    if dia in wm_const.wm_FLAT_LENGTHS:
        ...
        arc = FloatCanvas.Arc((x, y),
                              (-x, y),
                              (0, 0),
                              LineColor=wm_const.wm_WAFER_EDGE_COLOR.GetIM(),
                              LineWidth=3,
                              )
...
def draw_crosshairs(dia=150, dot=False):
    ...
    if dot:
        circ = FloatCanvas.Circle((0, 0),
                                  2.5,
                                  FillColor=wm_const.wm_WAFER_CENTER_DOT_COLOR.GetIM(),
                                  )
...
def draw_die_gridlines(wf):
    ...
    lines = [FloatCanvas.Line(l, LineColor=grey.GetIM()) for l in line_coords]
