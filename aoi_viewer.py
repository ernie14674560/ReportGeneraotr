...
class ViewerPanel(wx.Panel):
    ...
    # ----------------------------------------------------------------------
    def searchImage(self, Y, X, return_img_name=False, fill_num=3):
        # Y = self.editY.GetValue()
        # X = self.editX.GetValue()
        strY = f"{Y:.0f}".zfill(fill_num)
        strX = f"{X:.0f}".zfill(fill_num)
        ...
        try:
            ...
            if fill_num == 3:
                self.searchImage(Y, X, return_img_name, fill_num=2)
            else:
    ...
    def search_specific_bin(self, event):
        ...
        for X, Y, DATA in df.itertuples(index=False):
            img = self.searchImage(Y, X, return_img_name=True)
            if img in self.original_picPaths:
                img_list.append(self.searchImage(Y, X, return_img_name=True))
        # self.original_picPaths = self.picPaths
    ...
    def reset_to_default_paths(self, event):
        if self.original_picPaths:
            self.picPaths = self.original_picPaths
...
def main():
    app = wx.App()
    frame = AOIViewerFrame(default_path='\\\\192.168.7.27\\\\aoi3\\1NKK711#04_FrontSide', yx=(1, 24))
    app.MainLoop()
